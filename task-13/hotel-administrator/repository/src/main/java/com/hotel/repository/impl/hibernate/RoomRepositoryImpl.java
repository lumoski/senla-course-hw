...
@Slf4j
public class RoomRepositoryImpl extends AbstractJpaRepository<RoomEntity, Long> implements RoomRepository<RoomEntity, Long> {
    ...
    public List<RoomEntity> findAvailableRoomsByDate(LocalDate date) {
        ...
        throw new UnsupportedOperationException("Unimplemented method 'findAvailableRoomsByDate'");
    }
        ...
        try {
            ...
            if (sortField != null) {
                query.orderBy(ascending ? 
                    cb.asc(room.get(sortField)) : 
                    cb.desc(room.get(sortField)));
            }
